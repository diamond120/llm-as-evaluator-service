...
class InputSchema(BaseModel):
    conversation: list[dict] = Field(..., description="The conversation data.")


class CoTBase(BaseModel):
    plan_steps: list[str] = Field(
        ...,
        description="Executable numbered steps necessary to achieve the goal.",
    )
    scratchpad: str = Field(
        ...,
        description="Execute the task step by step.",
    )
...
class Issue(BaseModel):
    ...
    cell_pos: int = Field(..., description="if not available use -1")
    where: str = Field(
        ...,
        description="Text with the grammar mistake. Only related piece, MINIMUM CHARACTERS TO IDENTIFY.",
    )
    type: str = Field(..., description="Type of the issue identified.")
    fix: str = Field(
        ...,
        description="Suggested solution for the mistake, concise. It must provide the fix for the mistake, fixed version, not be copy paste of `where` field!",
    )


class OutputSchema(CoTBase):
    """Evaluation result for a single cell"""

    issues: list[Issue] = Field(
        ...,
        description="A list of issues in the cell, each described with where, type, and fix.",
    )
    score: int = Field(
        ...,
        description="score between 1 and 5, if more than 3 issues, score 2 or 1, the rest is up to you.",
    )
...
class ConfigSchema(BaseModel):
    ...
    worker_config: SingleStageMessagesEvaluator.ConfigSchema = Field(
        ..., description="Worker stage config(multiples)."
    )
    uniter_config: SingleStageMessagesEvaluator.ConfigSchema = Field(
        ..., description="Uniter stage config(many inputs one output)."
    )
